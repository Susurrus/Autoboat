...
#define MAVLINK_MSG_ID_GPS_STATUS_LEN 1
...
{
	return mavlink_msg_gps_status_pack(system_id, component_id, msg, gps_status->satellites_visible);
}
...
static inline void mavlink_msg_gps_status_send(mavlink_channel_t chan, uint8_t satellites_visible)
{
	...
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_GPS_STATUS, buf, 1, 130);
#else
	mavlink_gps_status_t packet;
	packet.satellites_visible = satellites_visible;

	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_GPS_STATUS, (const char *)&packet, 1, 130);
#endif
}
...
#else
	memcpy(gps_status, _MAV_PAYLOAD(msg), 1);
#endif
}
