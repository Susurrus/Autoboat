...
% Here we check for any jumps in the 
...
command_r = Simulink.Timeseries;
command_r.Name = 'command_r';
command_r.Time = autorudder_time;
command_r.Data = single(ranged_auto_rudder(valid_auto_rudder_data));
...
% And the autopilot's commnaded prop speed from BASIC_STATE
ranged_command_t = data.BASIC_STATE.commanded_auto_throttle(valid_range);
valid_command_t_data = ~isnan(ranged_command_t);
command_t_time = valid_range_time(valid_command_t_data);
command_t = Simulink.Timeseries;
command_t.Name = 'command_t';
command_t.Time = command_t_time;
command_t.Data = int16(ranged_command_t(valid_command_t_data));
